 ...
 */
import { directive, html, noChange, nothing, render, svg, templateFactory } from '../../lit-html.js';
import { stripExpressionMarkers } from '../test-utils/strip-markers.js';
const assert = chai.assert;
/* eslint-disable @typescript-eslint/no-explicit-any */
const isTemplatePolyfilled = (HTMLTemplateElement.decorate != null ||
    window.ShadyDOM && window.ShadyDOM.inUse);
const testSkipForTemplatePolyfill = (test) => isTemplatePolyfilled ? test.skip : test;
/* eslint @typescript-eslint/camelcase: ["error", { allow: ["Safari10_0"] }] */
const isSafari10_0 = (window.navigator.userAgent.indexOf('AppleWebKit/602') !== -1);
const testSkipSafari10_0 = (test) => isSafari10_0 ? test.skip : test;
const isChrome41 = (window.navigator.userAgent.indexOf('Chrome/41') !== -1);
const testSkipChrome41 = (test) => isChrome41 ? test.skip : test;
const testIfHasSymbol = (test) => window.Symbol === undefined ? test.skip : test;
const ua = window.navigator.userAgent;
const isIe = ua.indexOf('Trident/') > 0;
const suiteIfCustomElementsAreSupported = (suite) => (window.customElements != null) ? suite : suite.skip;
suite('render()', () => {
    let container;
    setup(() => {
    ...
    suite('text', () => {
        ...
        test('renders a number', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>123</div>');
        });
        ...
        test('renders nested templates', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<h1>foo</h1>bar');
        });
        ...
        test('renders parts that look like attributes', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>foo bar=baz</div>');
        });
        ...
        test('renders a template nested multiple times', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<h1>foo</h1>bar<h1>foo</h1>bazqux');
        });
        ...
        // html tag.
        test.skip('overwrites an existing (plain) TemplateInstance if one exists, ' +
            'even if it has a matching Template', () => {
            const t = (tag) => tag `<div>foo</div>`;
        ...
        test('renders an element', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div><p></p></div>');
        });
        test('renders an array of elements', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div><p></p><a></a><span></span></div>');
        });
        test('renders forms as elements', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<form><input name="one"><input name="two"></form>');
        });
        test('renders SVG', () => {
        ...
        test('renders SVG with bindings', () => {
            ...
            assert.include(['M7 14l5-5 5 5z', 'M 7 14 l 5 -5 l 5 5 Z'], path.getAttribute('d'), pathDefinition);
        });
        ...
        test('renders expressions with preceding elements', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<a>foo</a><h1>bar</h1>');
            // This is nearly the same test case as above, but was causing a
            // different stack trace
            render(html `<a>${'foo'}</a>${'bar'}`, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<a>foo</a>bar');
        });
    });
    suite('attributes', () => {
        ...
        testIfHasSymbol(test)('renders a Symbol to an attribute', () => {
            ...
            assert.include(container.querySelector('div').getAttribute('foo').toLowerCase(), 'symbol');
        });
        testIfHasSymbol(test)('renders a Symbol in an array to an attribute', () => {
            render(html `<div foo=${[Symbol('A')]}></div>`, container);
            assert.include(container.querySelector('div').getAttribute('foo').toLowerCase(), 'symbol');
        });
        test('renders multiple bindings in an attribute', () => {
            render(html `<div foo="a${'b'}c${'d'}e"></div>`, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div foo="abcde"></div>');
        });
        test('renders two attributes on one element', () => {
            const result = html `<div a="${1}" b="${2}"></div>`;
            render(result, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div a="1" b="2"></div>');
        });
        test('renders a binding in a style attribute', () => {
            const t = html `<div style="color: ${'red'}"></div>`;
            render(t, container);
            if (isIe) {
                assert.equal(stripExpressionMarkers(container.innerHTML), '<div style="color: red;"></div>');
            }
            else {
                assert.equal(stripExpressionMarkers(container.innerHTML), '<div style="color: red"></div>');
            }
        ...
        test('renders a binding in a class attribute', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div class="red"></div>');
        });
        test('renders a binding in an input value attribute', () => {
            render(html `<input value="${'the-value'}">`, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<input value="the-value">');
        ...
        test('renders a case-sensitive attribute', () => {
            ...
            assert.include(stripExpressionMarkers(container.innerHTML), 'viewBox="0 0 100 100"');
            // Make sure non-alpha valid attribute name characters are handled
            render(html `<svg view_Box="0 0 ${size} ${size}"></svg>`, container);
            assert.include(stripExpressionMarkers(container.innerHTML), 'view_Box="0 0 100 100"');
        });
        test('renders to an attribute expression after an attribute literal', () => {
            ...
            assert.include(['<div a="b" foo="bar"></div>', '<div foo="bar" a="b"></div>'], stripExpressionMarkers(container.innerHTML));
        });
        ...
        test('renders attribute binding after attribute binding that moved', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), `<a class="bar" href="foo"><div id="a"></div></a>`);
        });
        ...
        test('renders to attributes with attribute-like values', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div foo="bar=foo"></div>');
        });
        ...
        test('renders an array to an attribute', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div foo="123"></div>');
        });
        ...
        test('renders to an attribute after a node', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>baz</div><div foo="bar"></div>');
        });
        test('renders undefined in attributes', () => {
            render(html `<div attribute="it's ${undefined}"></div>`, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div attribute="it\'s undefined"></div>');
        });
    });
    suite('properties', () => {
    ...
    suite('boolean attributes', () => {
        test('renders a boolean attribute as an empty string when truthy', () => {
            const t = (value) => html `<div ?foo="${value}"></div>`;
    ...
    suite('events', () => {
        ...
        test('adds event listener functions, calls with right this value', () => {
            ...
            const eventContext = {}; // eslint-disable-line
    ...
    suite('directives', () => {
        test('renders directives on NodeParts', () => {
            const fooDirective = directive(() => (part) => {
                part.setValue('foo');
            });
        ...
        test('renders directives on PropertyParts', () => {
            ...
            render(html `<div .foo="${fooDirective()}"></div>`, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
            assert.strictEqual(container.firstElementChild.foo, 1234);
        });
        testSkipChrome41(test)('event listeners can see events fired by dynamic children', () => {
        ...
        test('event listeners can see events fired directives in AttributeParts', () => {
            ...
            let event = undefined;
            const fire = directive(() => (part) => {
                part.committer.element.dispatchEvent(new CustomEvent('test-event', {
                    bubbles: true,
                }));
            });
            render(html `<div @test-event=${(e) => {
                event = e;
            }} b=${fire()}></div>`, container);
            assert.isOk(event);
        });
    });
    suite('<table>', () => {
        testSkipForTemplatePolyfill(test)('renders nested templates within table content', () => {
            let table = html `<table>${html `<tr>${html `<td></td>`}</tr>`}</table>`;
            render(table, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<table><tr><td></td></tr></table>');
            table = html `<tbody>${html `<tr></tr>`}</tbody>`;
            render(table, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<tbody><tr></tr></tbody>');
            table = html `<table><tr></tr>${html `<tr></tr>`}</table>`;
            render(table, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<table><tbody><tr></tr><tr></tr></tbody></table>');
            table = html `<table><tr><td></td>${html `<td></td>`}</tr></table>`;
            render(table, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<table><tbody><tr><td></td><td></td></tr></tbody></table>');
            table = html `<table><tr><td></td>${html `<td></td>`}${html `<td></td>`}</tr></table>`;
            render(table, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<table><tbody><tr><td></td><td></td><td></td></tr></tbody></table>');
        });
        // On Safari 10.0 (but not 10.1), the attribute value "<table>" is
        // escaped to "&lt;table&gt;". That shouldn't cause this test to
        // fail, so we skip
        testSkipSafari10_0(test)('renders quoted attributes with "<table>" before an expression', () => {
            const template = html `<div a="<table>${'foo'}"></div>`;
            render(template, container);
            assert.equal(stripExpressionMarkers(container.innerHTML), `<div a="<table>foo"></div>`);
        });
    });
    suite('<style>', () => {
    ...
    suiteIfCustomElementsAreSupported(suite)('custom elements', () => {
        ...
        customElements.define('mutates-in-constructor', MutatesInConstructorElement);
        teardown(() => {
        ...
        test('uses property setters for custom elements', () => {
            ...
            document.body.appendChild(container);
            render(html `<property-tester .value=${'foo'}></property-tester>`, container);
        ...
        test('uses property setters in nested templates added after the initial render', () => {
            // Container must be in the document for the custom element to upgrade
            document.body.appendChild(container);
            const template = (content) => html `${content}`;
            // Do an initial render
            render(template('some content'), container);
            // Now update the rendered template, render a nested template
            const fragment = html `<property-tester .value=${'foo'}></property-tester>`;
            render(template(fragment), container);
            const instance = container.firstElementChild;
            assert.equal(instance.value, 'foo');
            assert.isTrue(instance.calledSetter);
        });
        test('uses property setters in disconnected nested templates added after the initial render', () => {
            // Here we _don't_ connect the container to the document, to highlight
        ...
        // this.
        test.skip('does not upgrade elements until after parts are established', () => {
            render(html `
    ...
    suite('updates', () => {
        ...
        test('updates when called multiple times with arrays', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<ul><li>a</li><li>b</li><li>c</li></ul>');
            render(ul(['x', 'y']), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<ul><li>x</li><li>y</li></ul>');
        });
        ...
        test('renders to and updates sibling parts', () => {
            ...
            foo = 'bbb';
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>bbbbar</div>');
        });
        test('renders and updates attributes', () => {
            ...
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div a="foo:bar"></div>');
            foo = 'bbb';
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div a="bbb:bar"></div>');
        });
        test('updates nested templates', () => {
            ...
            render(t(true), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<h1>foo</h1>baz');
            foo = 'bbb';
            render(t(true), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<h1>bbb</h1>baz');
            render(t(false), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<h2>bar</h2>baz');
        });
        test('updates arrays', () => {
            let items = [1, 2, 3];
            ...
            items = [3, 2, 1];
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>321</div>');
        });
        test('updates arrays that shrink then grow', () => {
            ...
            items = [5, 6, 7];
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>567</div>');
        });
        test('updates an element', () => {
            let child = document.createElement('p');
            const t = () => html `<div>${child}<div></div></div>`;
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div><p></p><div></div></div>');
            child = undefined;
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div><div></div></div>');
            child = document.createTextNode('foo');
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>foo<div></div></div>');
        });
    ...
    suite('accepts types other than TemplateResult', () => {
        ...
        test('accepts a node', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>text in the div</div>');
        });
    });
});
//# sourceMappingURL=render_test.js.map