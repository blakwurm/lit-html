...
suite('add/remove nodes from template', () => {
    ...
    test('inserting nodes into template between parts renders/updates result', () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), `<div><span>1</span></div>
        <div foo="bar">
          baz
    ...
    test('inserting documentFragment into template', () => {
        const getResult = (a, b, c) => html `<div foo="${a}">${b}<p>${c}</p></div>`;
        const result = getResult('bar', 'baz', 'qux');
        const template = templateFactory(result);
        const fragment1 = document.createDocumentFragment();
        fragment1.appendChild(document.createElement('div'));
        fragment1.firstChild.innerHTML =
            policy.createHTML('<span>1</span>');
        insertNodeIntoTemplate(template, fragment1, template.element.content.firstChild);
        const fragment2 = document.createDocumentFragment();
        insertNodeIntoTemplate(template, fragment2, template.element.content.querySelector('p'));
        render(result, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), `<div><span>1</span></div><div foo="bar">baz<p>qux</p></div>`);
        render(getResult('a', 'b', 'c'), container);
        assert.equal(stripExpressionMarkers(container.innerHTML), `<div><span>1</span></div><div foo="a">b<p>c</p></div>`);
    });
    test('removing nodes in template between parts renders/updates result', () => {
        const getResult = (a, b, c) => html `<div name="remove"><span>remove</span></div>
    ...
    test('removing nodes in template containing parts renders/updates result', () => {
        const getResult = (a, b, c, r1, r2, r3) => html `<div name="remove"><span>${r1}</span></div>
    ...
    test('removing nodes in template containing parts with in-active parts renders/updates result', () => {
        const getResult = (a, b, c, r1, r2, r3) => html `<div name="remove"><span>${r1}</span></div>
        <div foo="${a}"><div name="remove"><span>${r2}</span></div>
          ${b}
          <div name="remove"><span name="remove">${r3}</span></div><p>${c}</p>
...
//# sourceMappingURL=modify-template_test.js.map