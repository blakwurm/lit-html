 ...
 */
import { AttributeCommitter, createMarker, DefaultTemplateProcessor, directive, EventPart, html, NodePart, render, templateFactory, TemplateResult } from '../../lit-html.js';
    ...
    suite('NodePart', () => {
        ...
        suite('setValue', () => {
            ...
            test('accepts nested arrays', () => {
                ...
                part.commit();
                assert.equal(stripExpressionMarkers(container.innerHTML), '123');
                assert.deepEqual(Array.from(container.childNodes).map((n) => n.nodeValue), ['', '', '1', '', '', '2', '', '', '3', '', '']);
                assert.strictEqual(container.firstChild, startNode);
                assert.strictEqual(container.lastChild, endNode);
            });
            test('accepts nested templates', () => {
                ...
                part.commit();
                assert.equal(stripExpressionMarkers(container.innerHTML), '<h1>foo</h1>');
            });
            ...
            test('accepts an array of elements', () => {
                ...
                part.commit();
                assert.equal(stripExpressionMarkers(container.innerHTML), '<p></p><a></a><span></span>');
            });
            test('nested TemplateResults use their own processor', () => {
                ...
                class TestTemplateProcessor extends DefaultTemplateProcessor {
                    handleAttributeExpressions(element, name, strings) {
                        if (name[0] === '&') {
                            return super.handleAttributeExpressions(element, name.slice(1), strings, { templateFactory });
                        }
                        return super.handleAttributeExpressions(element, name, strings, { templateFactory });
                    }
                }
                const processor = new TestTemplateProcessor();
                const testHtml = (strings, ...values) => new TemplateResult(strings, values, 'html', processor);
            ...
            test('updates a simple value to a complex one', () => {
                ...
                render(t(), container);
                assert.equal(stripExpressionMarkers(container.innerHTML), '<div><span>bar</span></div>');
            });
            ...
            test('updates when called multiple times with arrays 2', () => {
                ...
                assert.deepEqual(Array.from(container.childNodes).map((n) => n.nodeValue), ['', '', '4', '', '5', '', '']);
                assert.strictEqual(container.firstChild, startNode);
                assert.strictEqual(container.lastChild, endNode);
                part.setValue([]);
                part.commit();
            ...
            test('updates arrays with siblings', () => {
                ...
                render(t(), container);
                assert.equal(stripExpressionMarkers(container.innerHTML), '<p></p>123<a></a>');
                items = [1, 2, 3, 4];
                render(t(), container);
                assert.equal(stripExpressionMarkers(container.innerHTML), '<p></p>1234<a></a>');
            });
            test('updates are stable when called multiple times with templates', () => {
                ...
                part.setValue(r());
                part.commit();
                assert.equal(stripExpressionMarkers(container.innerHTML), '<h1>bar</h1>');
            ...
            test('updates are stable when called multiple times with arrays of templates', () => {
                ...
                part.setValue(r());
                part.commit();
                assert.equal(stripExpressionMarkers(container.innerHTML), '<li>1</li><li>2</li><li>3</li>');
                const originalLIs = Array.from(container.querySelectorAll('li'));
                items = [3, 2, 1];
                part.setValue(r());
                part.commit();
                assert.equal(stripExpressionMarkers(container.innerHTML), '<li>3</li><li>2</li><li>1</li>');
                const newLIs = Array.from(container.querySelectorAll('li'));
                assert.deepEqual(newLIs, originalLIs);
            });
        });
        suite('insertAfterNode', () => {
            test('inserts part and sets values between ref node and its next sibling', () => {
                ...
                testPart.commit();
                assert.deepEqual(Array.from(container.childNodes), [startNode, endNode, text, testEndNode]);
            });
        });
        ...
        suite('insertAfterPart', () => {
            test('inserts part and sets values after another part', () => {
                ...
                testPart.setValue(text);
                testPart.commit();
                assert.deepEqual(Array.from(container.childNodes), [startNode, testPart.startNode, text, endNode]);
                const previousText = document.createTextNode('');
                part.setValue(previousText);
                part.commit();
                assert.deepEqual(Array.from(container.childNodes), [startNode, previousText, testPart.startNode, text, endNode]);
            });
        });
        suite('clear', () => {
        ...
        suite('directive holds NodePart', () => {
            test('does not throw errors when part is detached', () => {
                let part;
                const fooDirective = directive(() => (p) => {
                    part = p;
                });
                const t = (bool) => html `<div>${bool ? html `${fooDirective()}` : 'detached'}</div>`;
...
//# sourceMappingURL=parts_test.js.map