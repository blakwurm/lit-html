...
suite('shady-render scoping shim', () => {
    ...
    test(testName, function () {
        ...
        window.ShadyCSS.ScopingShim.prepareAdoptedCssText([':host { border-top: 2px solid black; }'], 'scope-1');
    ...
    test(testName, function () {
        ...
        window.ShadyCSS.ScopingShim.prepareAdoptedCssText([':host { border-top: 2px solid black; } button { font-size: 7px; } '], 'scope-2');
        document.body.appendChild(container);
        renderShadowRoot(undefined, container);
        assert.equal(getComputedStyle(container).getPropertyValue('border-top-width').trim(), '2px');
    ...
    test(testName, function () {
        ...
        style.innerHTML = policy.createHTML(':host { border-top: 2px solid black; } button { font-size: 7px; }');
...
//# sourceMappingURL=shady-render-scoping-shim_test.js.map