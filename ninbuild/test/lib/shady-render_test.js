...
suite('shady-render', () => {
    ...
    test('styles with css custom properties flow to multiple instances of nested shadowRoots', async () => {
        ...
        assert.equal(getComputedStyle(elements[0])
            .getPropertyValue('border-top-width')
            .trim(), '2px');
        assert.equal(getComputedStyle(elements[1])
            .getPropertyValue('border-top-width')
            .trim(), '2px');
        document.body.removeChild(container);
    });
    test('parts around styles with parts render/update', () => {
        ...
        document.body.appendChild(container);
        const renderTemplate = (border, a, b, c) => {
    ...
    test('parts around styles with parts render/update when stamped into muliple containers', () => {
        ...
        document.body.appendChild(container);
        const renderTemplate = (border, a, b, c, host = container) => {
            const result = html `<style></style><div id="a">${a}</div>
        <style>
          div {
            border: ${border};
      ...
      `;
        ...
        renderTemplate('1px solid black', '', '', '', document.createElement('scope-3b'));
    ...
    test('empty styles are ok', function () {
        ...
        const renderTemplate = (foo, container) => {
            const result = html `<div id="a">${foo}</div><style></style><div id="b">${foo}</div>`;
            renderShadowRoot(result, container);
        };
        renderTemplate('foo', container1);
        assert.equal(container1.shadowRoot.querySelector('#a').textContent, `foo`);
        assert.equal(container1.shadowRoot.querySelector('#b').textContent, `foo`);
        const container2 = document.createElement('scope-empty-style');
        document.body.appendChild(container2);
        renderTemplate('bar', container2);
        assert.equal(container2.shadowRoot.querySelector('#a').textContent, `bar`);
        assert.equal(container2.shadowRoot.querySelector('#b').textContent, `bar`);
        document.body.removeChild(container1);
        document.body.removeChild(container2);
    });
    ...
    test('throws an error if the options argument is not an object with a `scopeName` property', () => {
        const container = document.createElement('some-element');
        document.body.appendChild(container);
        const shadowRoot = container.attachShadow({ mode: 'open' });
        let error1 = undefined;
        try {
            shadyRender(html `some content`, shadowRoot /*, not provided */);
        }
        catch (e) {
            error1 = e;
        }
        ...
        try {
            shadyRender(html `some content`, shadowRoot, 'not an object');
        }
        catch (e) {
            error2 = e;
        }
        ...
        try {
            shadyRender(html `some content`, shadowRoot, { 'missing scopeName': true });
        }
        catch (e) {
            error3 = e;
        }
    ...
    test('parts around <slot> elemnets', () => {
        ...
        render('foo');
        assert.equal((_a = el.shadowRoot) === null || _a === void 0 ? void 0 : _a.textContent, ' foo');
        render('bar');
        assert.equal((_b = el.shadowRoot) === null || _b === void 0 ? void 0 : _b.textContent, ' bar');
        render('');
        assert.equal((_c = el.shadowRoot) === null || _c === void 0 ? void 0 : _c.textContent, ' ');
        render('zot');
        assert.equal((_d = el.shadowRoot) === null || _d === void 0 ? void 0 : _d.textContent, ' zot');
        const c1 = document.createElement('div');
        c1.setAttribute('slot', 'before');
        el.appendChild(c1);
        assert.equal((_e = el.shadowRoot) === null || _e === void 0 ? void 0 : _e.textContent, ' zot');
        render('c1');
        assert.equal((_f = el.shadowRoot) === null || _f === void 0 ? void 0 : _f.textContent, ' c1');
        const c2 = document.createElement('div');
        c2.setAttribute('slot', 'after');
        el.appendChild(c2);
        render('c1c2');
        assert.equal((_g = el.shadowRoot) === null || _g === void 0 ? void 0 : _g.textContent, ' c1c2');
        el.textContent = '';
        assert.equal((_h = el.shadowRoot) === null || _h === void 0 ? void 0 : _h.textContent, ' c1c2');
        document.body.removeChild(el);
    });
});
//# sourceMappingURL=shady-render_test.js.map