...
suite('repeat', () => {
    ...
    suite('keyed', () => {
        ...
        test('renders a list twice', () => {
            const t = (items) => html `${repeat(items, (i) => i, (i) => html `
            <li>item: ${i}</li>`)}`;
    ...
    suite('rendering other values', () => {
        test('render promises as values', async () => {
            const items = [0, 1, 2];
            const t = () => html `${repeat(items, (i) => until(Promise.resolve(html `
            <li>promised: ${i}</li>`)))}`;
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '');
        ...
        test('render nested directives as values', async () => {
            const items = [0, 1, 2];
            const t = () => html `${repeat(items, (i) => until(Promise.resolve(guard(items, () => html `
            <li>guarded: ${i}</li>`)), html `
            <li>wait: ${i}</li>`))}`;
            render(t(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), `
            <li>wait: 0</li>
