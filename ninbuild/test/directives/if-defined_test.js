...
suite('ifDefined', () => {
    ...
    test('removes an attribute with multiple undefined values', () => {
        render(html `<div foo="they're both: ${ifDefined(undefined)}${ifDefined(undefined)}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
    });
    test('removes an attribute with one defined then one undefined value', () => {
        render(html `<div foo="only one is: ${ifDefined('a')}${ifDefined(undefined)}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
    });
    // TODO(justinfagnani): fix this, see
    // https://github.com/Polymer/lit-html/issues/1066
    test.skip('removes an attribute with one undefined then one defined value', () => {
        render(html `<div foo="only one is: ${ifDefined(undefined)}${ifDefined('a')}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
    });
